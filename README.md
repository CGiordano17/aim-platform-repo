...
**Not built yet:** the **Workflows** tab (referenced in PRD §3's nav table, but never implemented even in the original prototype — `design/active/workflows-interventions.html` is a mockup only) and the **Integrations registry** (PRD build phase 6 — schema reserved in `public.integrations`, no UI, no vendor chosen). Deep system integrations (Tier 3, phase 7) come after that.
...
src/
  app/                          — Next.js App Router
    ...
    (app)/                      — protected app shell (role-gated nav)
      goals/, survey/, assessments/, teams/, nudges/  — pages + their server actions
...
design/
  active/                       — mockups that still represent current, un-superseded decisions
                                   (workflows-interventions.html has no real implementation yet)
...
1. Create a Supabase project, then run both migrations in order:
   `supabase db push` (with the Supabase CLI linked to your project), or paste
   `supabase/migrations/0001_init.sql` then `0002_tier1_review_log.sql` into the SQL editor.
...
5. Stand up a real Vercel + Supabase project and browser-test the whole thing — nothing here has run outside `npm run build` yet.
6. Build the **Workflows** tab — referenced in PRD §3's nav but never implemented (not one of the numbered PRD phases, a separate gap).
7. Build the Integrations registry framework + first vendor connector (phase 6, no vendor chosen yet).
8. Build deep system integrations (Tier 3, phase 7).
